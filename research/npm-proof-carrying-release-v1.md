...
2. Pin the canonical supply-chain verification toolchain to Node 24 and npm 11.19.0. The published runtime contract is `engines.node >=20` because the locked graph includes `@vercel/oidc@3.1.0` with `node >= 20`; claiming Node 18 would exceed the dependency evidence. A separate engine-strict matrix replays build, pack, isolated install, public API load, and CLI execution on Node 20/22/24 while pinning the same npm 11.19.0 toolchain on all three runtimes; it requires all three runtimes to produce the same tarball SHA-256 before release authority can proceed. GitHub action dependencies are also pinned to signed Node24-native release commits: checkout v7.0.1, setup-node v7.0.0, upload-artifact v7.0.1, and download-artifact v8.0.1.
