...
2. Pin the canonical supply-chain verification toolchain to Node 24 and npm 11.19.0. The published runtime contract is `engines.node >=20` because the locked graph includes `@vercel/oidc@3.1.0` with `node >= 20`; claiming Node 18 would exceed the dependency evidence. A separate engine-strict matrix replays build, pack, isolated install, public API load, and CLI execution on Node 20/22/24 while pinning the same npm 11.19.0 toolchain on all three runtimes; it requires all three runtimes to produce the same tarball SHA-256 and requires that matrix digest to equal the canonical Node-24 verify-package tarball digest before release authority can proceed. GitHub action dependencies are also pinned to signed Node24-native release commits: checkout v7.0.1, setup-node v7.0.0, upload-artifact v7.0.1, and download-artifact v8.0.1.
...
5. Produce two independent `npm pack --json --ignore-scripts` outputs and require byte-for-byte equality, equal integrity metadata, and equal file census. Independently enumerate each concrete `.tgz` with `tar -tf`/`tar -tvf`, require every entry to be a regular file with safe non-group/world-writable permissions and a safe path rooted under `package/`, restrict the publish surface to `dist/**`, `README.md`, and `package.json`, and require the stripped tar census to match npm's metadata census exactly.
6. Packed name/version must equal the committed manifest, the entire packed `package.json` must be canonically JSON-equal to the committed manifest, and the public package surface is fixed to `main=./dist/src/index.js`, `types=./dist/src/index.d.ts`, both CLI bins at `./dist/bin/guard.js`, `files=["dist/**","README.md"]`, and public provenance-enabled publishConfig. Those entrypoints must be present in the concrete tarball and the isolated installed package must load the declared public API. The tarball filename must be canonical `sovereign-guard-<version>.tgz`, and install lifecycle hooks must be absent.
