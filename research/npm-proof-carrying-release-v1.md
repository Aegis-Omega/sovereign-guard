...
9. Registry release runs only on the GitHub `release.published` boundary. The release tag is admissible only when its exact commit is contained in the canonical `master` history; a draft-save/create event or side-branch tag cannot reach registry action merely by passing package verification.
10. Release must reuse the verified tarball. Before registry action, the committed and PR-tested `scripts/npm-release-preflight.mjs` recomputes both canonical receipt roots and requires package receipt source SHA = supply-chain receipt source SHA = admitted release SHA, verifies the concrete tarball bytes, and rechecks hosted run identity. The workflow fails closed unless `NPM_RELEASE_MODE` explicitly selects `trusted-stage` or `bootstrap-token`; OIDC permission exists only on the registry-action job.
