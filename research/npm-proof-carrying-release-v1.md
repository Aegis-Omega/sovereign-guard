...
10. Release must reuse the verified tarball. The verify job emits the exact evidence producer `run_id`, `run_attempt`, attempt-bound artifact name, immutable GitHub artifact ID, and artifact digest; downstream registry action requires the producer metadata and downloads by immutable artifact ID, then validates against the producer identity rather than the consumer retry attempt. Before registry action, the committed and PR-tested `scripts/npm-release-preflight.mjs` verifies the uploaded SHA-256 evidence manifest, recomputes both canonical receipt roots, rebinds the actual audit/signature/SBOM files and checkout `package-lock.json` to their receipt digests, requires package receipt source SHA = supply-chain receipt source SHA = admitted release SHA, verifies the concrete tarball bytes, and rechecks hosted run identity. `scripts/npm-release-mode-guard.mjs` then observes npm registry package existence fail-closed: `trusted-stage` is allowed only for an existing package, while `bootstrap-token` is allowed only for first publication when the package is absent. OIDC permission exists only on the registry-action job.
