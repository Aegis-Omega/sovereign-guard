...
6. Packed name/version must equal the committed manifest, the entire packed `package.json` must be canonically JSON-equal to the committed manifest, the tarball filename must be canonical `sovereign-guard-<version>.tgz`, and install lifecycle hooks must be absent.
